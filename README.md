...
- **Профиль и резюме** — аватар, рейтинг, приватная вкладка «Личные», загрузка нескольких PDF-резюме, автоматическое заполнение структурированных разделов и выбор активного резюме
...
| Push-уведомления | iPhone/iPad PWA: standards-based Web Push + VAPID; Android: Expo Push → FCM |
...
**Почему PHP-прокси?**  
Прямой доступ к Supabase из браузера заблокирован в России, поэтому нативное приложение и веб-версия обращаются к российскому PHP-прокси на `jobtoo.ru`. Зарубежного резервного прокси нет.

**Паттерн вызова БД:**  
Все функции в `services/db.ts` имеют единообразный интерфейс и вызывают `proxy(fnName, args)`.
...
| Таблица | Назначение |
|---|---|
| `jm_users` | Профили работников и работодателей, публичная структура активного резюме и приватная личная анкета |
| `jm_resume_files` | Приватный сейф версий PDF-резюме и распознанных данных |
...
### Admin Dashboard
Дашборд работает на том же московском сервере (`admin.jobtoo.ru`).
