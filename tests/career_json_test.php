...
// ── Поле-объект и поле-список объектов ───────────────────────────────────────
// Найдено на живом ответе careers.yadro.com: город приходит не строкой, а
// списком [{"id":2,"name":"Москва"},…]. Прежняя версия читала такие поля через
// cf_text и молча теряла значение — город просто не появлялся.
$yadro = ['vacancies' => [[
    'id' => 2511, 'slug' => '102511', 'title' => 'SRE-инженер',
    'city' => [['id' => 2, 'name' => 'Москва'], ['id' => 3, 'name' => 'Санкт-Петербург']],
    'empl' => [['id' => 2, 'name' => 'Гибридный']],
    'direction' => ['id' => 14, 'name' => 'IT и разработка продуктов'],
]]];
$items4 = cf_json_items($yadro, [
    'list' => 'vacancies', 'title' => 'title', 'id' => 'id',
    'url_template' => 'https://careers.yadro.com/vacancy/{slug}',
    'address' => 'city', 'schedule' => 'empl', 'company' => 'direction',
], 'https://careers.yadro.com/vacancies', $now);
check('вакансия из живого формата прочитана', count($items4) === 1);
check('город взят из списка объектов', ($items4[0]['address'] ?? '') === 'Москва');
check('формат работы взят из списка объектов', ($items4[0]['schedule'] ?? '') === 'Гибридный');
check('поле-объект читается по name', ($items4[0]['company'] ?? '') === 'IT и разработка продуктов');
