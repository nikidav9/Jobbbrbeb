...
// Импорт проверяем по факту, а не по точному тексту строки: рядом с ReplyBadge
// приехал hasReplyBadge, и проверка на дословное совпадение покраснела, хотя
// плашка как была подключена, так и осталась.
check('плашка подключена',
    (bool)preg_match("~import \{[^}]*\bReplyBadge\b[^}]*\} from '@/components/feature/ReplyBadge';~", $feed));
...
// Разделитель под плашкой — приём вёрстки, а не обещание пользователю.
// Редизайн карточки 23.09 убрал его из тела (стиль cardDivider остался, но
// больше не используется), и эта проверка покраснела на ровном месте: она
// требовала не порядка, а наличия разделителя.
//
// Требование порядка сохранено ровно там, где оно про человека: плашка после
// чипов и ДО описания. Если разделитель когда-нибудь вернётся — проверка
// снова начнёт следить, что плашка выше него.
...
check('по одной переписке вывода не делаем',
    (bool)preg_match('~function hasReplyBadge\([^)]*\)[^{]*\{\s*return !!stats && stats\.chats >= 2;~', $badge));
check('плашка пользуется этим порогом, а не своим',
    str_contains($badge, 'if (!hasReplyBadge(stats)) return null;'));
// Порог ровно один. Считаем именно СРАВНЕНИЕ с числом, а не слово «chats»:
// оно встречается и в тексте плашки («Отвечает в 8 из 10»), и первая версия
// этой проверки покраснела на ровном месте. Второе сравнение рядом — это
// разъезд, который однажды случится молча.

// ── Обёртка не занимает место, когда плашки нет ─────────────────────────────
// ReplyBadge возвращает null при одной переписке, а обёртка с отступами
// оставалась всегда: между чипами и разделителем зияли 42 пикселя пустоты у
// КАЖДОГО работодателя без переписок. На карточке с данными этого не видно —
// поймано снимком с фикстурой, где данных нет.
// Комментарии вырезаем: порог упомянут словами прямо над функцией, и без
// этого проверка считала бы объяснение за вторую реализацию. Тот же приём
// в tests/jupiter_applications_test.php, и по той же причине.
$badgeCode = preg_replace('~/\*.*?\*/|//[^\n]*~s', '', $badge);
preg_match_all('~chats\s*(<|>=|<=|>)\s*\d~', $badgeCode, $thresholds);
check('порог задан в одном месте', count($thresholds[0]) === 1);

check('обёртка плашки условна',
    (bool)preg_match('~\{hasReplyBadge\(responsivenessMap\[v\.employerId\]\)\s*\?~', $feed));
