...
/**
 * Базовое имя цели и пропуск уже подключённых компаний.
 *
 * Разведка теперь запускается еженедельным таймером, и список без пропуска
 * каждый раз заново гонял бы браузером компании, у которых источник уже
 * работает.
 */
test('базовое имя — часть до « · раздел»', () => {
  assert.equal(baseName('Магнит · склад'), 'Магнит');
  assert.equal(baseName('  Пятёрочка  '), 'Пятёрочка');
  assert.equal(baseName('Без раздела'), 'Без раздела');
});

test('пропускает цель, чьё базовое имя уже в списке подключённых', () => {
  const targets = [
    { name: 'Магнит · склад', url: 'https://a.ru' },
    { name: 'Магнит · офис', url: 'https://b.ru' },
    { name: 'Пятёрочка', url: 'https://c.ru' },
  ];
  const got = skipTargets(targets, [' магнит ', '# комментарий не название']);
  assert.deepEqual(got, [{ name: 'Пятёрочка', url: 'https://c.ru' }]);
});

test('пустой список пропуска ничего не меняет', () => {
  const targets = [{ name: 'X', url: 'https://x.ru' }];
  assert.deepEqual(skipTargets(targets, []), targets);
  assert.deepEqual(skipTargets(targets, ['']), targets);
});

/**
 * Заглушки антибот-защиты и капчи. Разведка на них не должна ничего пробовать
...
test('капча на обычной странице 200 распознаётся', () => {
