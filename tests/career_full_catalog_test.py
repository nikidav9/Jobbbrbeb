...
# Endpoints приезжают файлом из репозитория, а не читаются из той же базы,
# которую заполняют. Прежняя схема стоила нам всего списка: миграция 096
# снесла таблицу, и единственный экземпляр списка исчез вместе с ней.
check('runtime получает проверенные endpoints', "scripts/career-endpoints.json" in sync)
check('endpoints не читаются обратно из базы', "connector_config->'endpoints'" not in sync)
