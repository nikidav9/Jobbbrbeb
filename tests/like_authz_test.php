...
check('уведомление уходит', str_contains($ann, "notify_bell(\$workerId, '❌ Отклик отклонён'"));
$notifyAt = strpos($ann, 'notify_bell($workerId');
...
// ── Отказ приходит ТИХО: колокольчик есть, пуша и телеграма нет ───────────────
// Отказов много и все они неприятные. Человек, разбуженный третьим за вечер,
// выключает уведомления целиком — и вместе с отказами перестаёт получать
// сообщения о мэтчах и о завтрашней смене, то есть ровно то, ради чего он их
// и включал. Выключенные уведомления не вернуть.
check('отказ шлётся колокольчиком, а не полным уведомлением',
    !str_contains($ann, 'notify_user($workerId'));
$bell = fn_body($db, 'notify_bell');
check('колокольчик есть', $bell !== '');
check('колокольчик пишет строку', str_contains($bell, "sb_insert('jm_notifications'"));
// Главное в этой проверке. Если колокольчик однажды начнёт слать пуш, тихий
// отказ станет громким, а тест об этом молчать не должен.
check('колокольчик не шлёт пуш', !str_contains($bell, 'expo_push('));
check('колокольчик не шлёт в телеграм', !str_contains($bell, 'tg_send_message('));
// А полное уведомление по-прежнему делает и то и другое — иначе тихим стал бы
// весь сервис разом, включая мэтчи.
$full = fn_body($db, 'notify_user');
check('полное уведомление опирается на колокольчик',
    str_contains($full, 'if (!notify_bell($userId, $title, $body, $type)) return;'));
check('полное уведомление шлёт пуш', str_contains($full, 'expo_push('));
check('полное уведомление шлёт в телеграм', str_contains($full, 'tg_send_message('));
// Дубль в пределах минуты гасится в колокольчике — значит и пуша не будет.
check('повтор гасится до отправки', str_contains($bell, 'if ($dup) return false;'));
// Заявка на постоянную вакансию — другое дело: её подают осознанно и ответа
// ждут, приходит она редко. Там пуш остаётся, и это решение, а не недосмотр.
$perm = fn_body($db, 'jt_perm_app_announce');
check('отказ по постоянной вакансии остаётся громким',
    str_contains($perm, "notify_user(\$workerId, '❌ Заявка отклонена'"));
