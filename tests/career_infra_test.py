...
# Отброшенный адрес исчезал молча: в настройке 24 источника, обход видел 23, и
# понять, какой выпал, было нечем. Заметил только сплошной проверкой на проде.
check("отброшенные адреса видны в ответе",
      "$skipped[] = $u['url'];" in career and "$out['skipped'] = $skipped;" in career)
...
# Реакция на непрошедший адрес — пропуск этой компании, а не обрыв всего
# обхода: ошибочный ответ приёмник понимает как «заход не удался» и упирается в
# тот же адрес каждый следующий раз. Сама проверка при этом обязана остаться.
check("адрес порции проверяется заново",
      "if (!ing_safe_https_url($pageUrl)) $skipUnit(" in career)
check("непрошедший адрес не рушит обход остальных",
      "cf_next_step($page, $sub, $total, false, true)" in career)
