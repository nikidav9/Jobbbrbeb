...
// Заголовки нарочно те же, что слало приложение: повтор с тем же заголовком в
// течение минуты гасится, и со старой сборки второго уведомления не придёт.
// Глушитель живёт в notify_bell — notify_user начинается с него, поэтому
// проверяем оба конца цепочки: один без другого её разрывает.
check('повтор уведомления гасится по заголовку',
    str_contains(fn_body($db, 'notify_bell'), "'title'      => 'eq.' . \$title"));
check('полное уведомление проходит через глушитель',
    str_contains(fn_body($db, 'notify_user'), 'if (!notify_bell($userId, $title, $body, $type)) return;'));
...
check('атомарное одобрение может отключить системную строку',
...
// Своего текста приложение не держит: он жил бы вторым местом и разъехался бы
// с серверным. Вместо дорисовки на месте — сразу перечитать переписку.
check('приложение своего текста о решении не держит',
    !str_contains($chat, 'Кандидат одобрен на вакансию')
    && !str_contains($chat, 'Вы не подошли по данной вакансии'));
check('вакансия названа',
    str_contains($db, "'Заявка на вакансию' . \$named . ' отклонена. Переписка по ней закрыта.'")
    && str_contains($db, "'Заявка на вакансию' . \$named . ' одобрена. Обсудите детали выхода.'"));
check('после решения переписка перечитывается',
    substr_count($chat, 'pollRef.current?.();') >= 3);
// Название вакансии могли стереть вместе с вакансией — тогда просто без него,
// иначе вышло бы «на вакансию «вакансию»».
check('без названия строка всё равно читается',
    str_contains(fn_body($db, 'jt_perm_app_announce'),
        "\$named = \$title !== '' ? ' «' . \$title . '»' : '';"));
