...
function ing_dedupe_key(array $v): string
{
    ...
    $address = $norm($permanent ? ($v['address'] ?? '') : '');
    $station = $norm($v['metro_station_norm'] ?? $v['metro_station'] ?? '');
    // А если работодатель не присылает НИ адреса, НИ метро, отпечаток
    // вырождается в «компания + должность» — и все вакансии сети с одинаковым
    // названием становятся одной карточкой. У METRO так пропадало 194 вакансии
    // из 294: двадцать три разных «Продавца-кассира» в разных магазинах
    // схлопывались в одну. Считано по базе, не на глаз.
    //
    // Тогда различаем по адресу самой вакансии: он у каждой свой. Сведение
    // одинаковых смен от РАЗНЫХ партнёров при этом не страдает — там место
    // есть всегда, иначе смену не показать ни на карте, ни в ленте.
    $noPlace = $permanent && $address === '' && $station === '';
 ...
 * Посчитано по проду: описания НЕ было у 725 вакансий из 1583.
 *
