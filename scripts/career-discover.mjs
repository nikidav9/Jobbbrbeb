 ...
 *
 * Запуск — недельный таймер на московском сервере (infra/career-discover-run.sh):
 * найденное проверяет php-proxy/career_verify.php и включает автоматически.
 ...
 *   DISCOVER_SKIP_FILE     путь к списку уже подключённых компаний (по строке),
 *                          такие цели пропускаются — см. skipTargets
 *
...
// каталога: запускают его и из корня репозитория, и из infra/career-discover-run.sh.
...
let targets = await loadTargets(process.argv[2]);
// Компании, у которых уже есть рабочий источник, второй раз смотреть незачем:
// таймер запускается еженедельно, и без этого разведка каждый раз проходила
// бы весь список заново.
const skipFile = process.env.DISCOVER_SKIP_FILE || '';
if (skipFile && fs.existsSync(skipFile)) {
  const before = targets.length;
  targets = skipTargets(targets, fs.readFileSync(skipFile, 'utf8').split('\n'));
  if (before > targets.length) console.log(`Пропущено уже подключённых: ${before - targets.length}`);
}
...
async function inspect(target, i) {
  ...
  try {
    const response = await page.goto(target.url, { waitUntil: 'domcontentloaded', timeout: TIMEOUT });
    // Антибот-защита и капча — сразу «закрыт», без попыток обойти: ждать
    // догрузку, крутить страницу и жать кнопки на заглушке незачем и не нужно.
    ...
    if (block) {
      ...
      console.log(`${label} закрыт: ${block}`);
      throw entry;
    }
    ...
    if (best) {
      ...
      // Источник же не сообщает своё имя сам — записываем его как константу,
      // тем же способом, что и для ссылок в разметке (см. ниже).
      if (!map.company_const) map.company_const = baseName(target.name);
      ...
      if (links) {
        entry = {
          ...
          connector_config: { endpoints: [{
            url: target.url,
            mode: 'html_links',
            map: { link_path: links.link_path, company_const: baseName(target.name), min_title: 8 },
          }] },
        };
    ...
    // «Закрыт» бросается сознательно (см. выше) — это не сбой, entry уже готов.
    if (e !== entry) {
      entry.status = 'ошибка';
...
const blocked = results.filter(r => r.status === 'закрыт');
...
console.log(`Закрыто защитой: ${blocked.length}`);
