...
/**
 * Базовое имя цели: часть названия до « · раздел», без пробелов по краям.
 *
 * У части компаний в career-sites.tsv несколько разделов одного сайта —
 * «Магнит · склад», «Магнит · офис». И проверка «уже подключено», и
 * company_const в найденном источнике должны знать компанию, а не раздел.
 */
...
/**
 * Убрать из списка целей компании, у которых уже есть рабочий источник.
 *
 * skipNames — сырые строки из файла: по одной компании на строку, регистр и
 * пробела по краям не важны. Сравнение идёт по baseName, чтобы отдельный
 * раздел уже подключённой компании не считался новой целью.
 */
export function skipTargets(targets, skipNames) {
  const skip = new Set((skipNames || []).map(n => baseName(n).toLowerCase()).filter(Boolean));
  if (!skip.size) return targets;
  return targets.filter(t => !skip.has(baseName(t.name).toLowerCase()));
}

/**
 * Признаки антибот-защиты или капчи — не обход, а честное «сюда не суёмся».
 *
 * Раньше такой ответ разведка читала как обычную страницу: крутила, жала
 * кнопки, искала JSON в заглушке — и это не только впустую тратило время, но
