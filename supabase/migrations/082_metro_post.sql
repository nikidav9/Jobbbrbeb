-- METRO: 292 вакансии через POST.
--
-- Их API на GET отвечает 405 Method Not Allowed, поэтому источник висел в
-- «не взято» с самого начала. Коннектор теперь умеет POST: метод и тело
-- задаются в настройке источника рядом с адресом (`method`, `body`).
--
-- Про безопасность, потому что POST — это новая возможность у точки входа,
-- открытой наружу. Тело берётся ТОЛЬКО из настройки, которую пишет
-- администратор, — из запроса в него не попадает ничего. Адрес проходит ту же
-- проверку, что и раньше: публичный HTTPS, без служебных сетей, без
-- переходов по редиректу, с закреплённым DNS-адресом. POST идёт тем же
-- curl-вызовом и под теми же сторожами, отдельного запроса мимо них нет.
-- Обе мутации (POST всегда включён; тело берётся из запроса) валят проверки.
--
-- Адрес вакансии проверен строго: /vacancy/21580 отдаёт страницу с названием
-- должности, а /vacancy/jobtoo-probe-404 — ту же оболочку БЕЗ него. Листание
-- через line/linesPerPage, вторая порция даёт другие номера (проверено).
--
-- Золотое Яблоко и Т-Банк тем же способом не открылись. У Золотого Яблока
-- поле `filters` ждёт список особого вида, у Т-Банка запрос принимается, но
-- возвращает ноль вакансий без правильных фильтров. Подобрать вслепую не
-- вышло, и гадать дальше я не стал — записано как есть.
