-- Пятёрочка и Перекрёсток: 38 тысяч вакансий линейного персонала.
--
-- Найдены прогоном разведки по нашему списку из 170 страниц. Это ровно тот
-- случай, ради которого разведка и писалась: обычным перебором типовых путей
-- такой адрес не найти — у Пятёрочки это
-- /api/vacancy/hire-request, у Перекрёстка тот же backend через /gw/.
--
-- Объём проверен запросом: у Пятёрочки total = 33905, у Перекрёстка 3998.
-- Для сравнения, весь каталог конкурента cofinder — 12 059 вакансий по всем
-- 111 компаниям. Это «Продавец-кассир» за 52 000, «Оператор склада» — то, чего
-- в разделе «Подработка» не было вовсе.
--
-- Разведка нашла адрес С ФИЛЬТРОМ города: radius=30000, cityId=692434fa…, и
-- пометила это полем filter_note — ради такого случая пометка и заводилась.
-- С фильтром отдавалось 10 вакансий вместо 33905. В настройке фильтра нет.
--
-- Адрес вакансии проверен, как после случая со Сбером. У Перекрёстка прямо:
-- /vacancies/{id} отдаёт 200 с названием должности, выдуманный id — 404.
-- У Пятёрочки страница рисуется скриптом и оболочка одинаковая, поэтому
-- проверено через их же API: /api/vacancy/hire-request/{id} возвращает
-- вакансию, а /000000000000000000000000 — 404. Номер настоящий, маршрут есть
-- (соседние пути /vacancies/ и /vakansii/ отвечают 404, значит сайт их
-- различает).
--
-- Что НЕ взято из того же прогона и почему:
--   METRO, Золотое Яблоко, Т-Банк — их API отвечает 405/404 на GET, нужен
--     POST, а коннектор ходит только GET. Отдельная работа.
--   Селектел, Globus IT, VK — список вакансий читается, но адрес вакансии
--     подобрать не удалось: все угаданные формы отдают 404.
--   Сбер — снова без адреса вакансии, как и было. Не берём.
