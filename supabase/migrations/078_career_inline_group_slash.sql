-- Inline Group: адрес со слэшем, как у IBS.
--
-- Сплошная проверка всех источников на проде: 22 из 23 отдают вакансии, один
-- падает на 301. Причина та же, что была у IBS: сервер редиректит
-- /about/jobs → /about/jobs/, а мы за редиректом не идём намеренно — он увёл бы
-- на адрес, который проверку не проходил.
--
-- Заодно из ленты уходят счётчики и разделы, замеченные тем же прогоном:
-- «2 вакансии» и «17 вакансий» у IBS, «Рекомендовать друга» у Контура,
-- «Разработчикам» у iSpring. Отсев теперь в коде, здесь только адрес.
--
-- И главное, что показал прогон: в настройке было 24 источника, а обход видел
-- 23 — МегаФон выпадал МОЛЧА, не пройдя проверку адреса на боевом сервере
-- (локально он её проходит). Теперь коннектор возвращает список отброшенных
-- адресов полем `skipped`, и такой случай больше не спрячется.
...
-- Счётчики и разделы, успевшие попасть в ленту.
delete from public.jm_ext_vacancies
 where source_id = 'career'
   and (title ~* '^[0-9]+\s+ваканс' or title in ('Рекомендовать друга', 'Разработчикам'));
